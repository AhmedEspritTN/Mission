...
class RegistrationServicesImplTest {
    ...
    @Test
    void testAddRegistrationAndAssignToSkierAndCourse_collectiveChildrenFullCourse() {
        Registration registration = new Registration();
        Skier skier = mock(Skier.class);
        when(skier.getNumSkier()).thenReturn(1L);
        when(skier.getDateOfBirth()).thenReturn(java.time.LocalDate.of(2015, 1, 1));
    ...
    @Test
    void testAddRegistrationAndAssignToSkierAndCourse_collectiveChildrenWrongAge() {
        Registration registration = new Registration();
        Skier skier = mock(Skier.class);
        when(skier.getNumSkier()).thenReturn(1L);
        when(skier.getDateOfBirth()).thenReturn(java.time.LocalDate.of(2000, 1, 1));
    ...
    @Test
    void testAddRegistrationAndAssignToSkierAndCourse_collectiveAdultFullCourse() {
        Registration registration = new Registration();
        Skier skier = mock(Skier.class);
        when(skier.getNumSkier()).thenReturn(1L);
        when(skier.getDateOfBirth()).thenReturn(java.time.LocalDate.of(1990, 1, 1));
    ...
    @Test
    void testAddRegistrationAndAssignToSkierAndCourse_collectiveAdultWrongAge() {
        Registration registration = new Registration();
        Skier skier = mock(Skier.class);
        when(skier.getNumSkier()).thenReturn(1L);
        when(skier.getDateOfBirth()).thenReturn(java.time.LocalDate.of(2010, 1, 1));
        Course course = mock(Course.class);
        when(course.getNumCourse()).thenReturn(2L);
        when(course.getTypeCourse()).thenReturn(tn.esprit.spring.entities.TypeCourse.COLLECTIVE_ADULT);
        registration.setNumWeek(1);
    ...
    @Test
    void testNumWeeksCourseOfInstructorBySupport_empty() {
        when(registrationRepository.numWeeksCourseOfInstructorBySupport(2L, Support.SNOWBOARD)).thenReturn(Collections.emptyList());
        List<Integer> result = registrationServices.numWeeksCourseOfInstructorBySupport(2L, Support.SNOWBOARD);
        assertNotNull(result);
        assertTrue(result.isEmpty());
        verify(registrationRepository).numWeeksCourseOfInstructorBySupport(2L, Support.SNOWBOARD);
    }
}
